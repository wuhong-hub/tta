...
当前包含: 初始科技(initial) + 时代 A(age_a); 时代 I/II/III 由
Task 10-12 扩充。engine 包不依赖本包, 卡牌仅以 handler 名字符串引用
effects.py 注册的处理器。
"""
...
def build_card_db() -> CardDB:
    """构建当前已实现的官方牌库(初始科技 + 时代 A)."""
    cards = {card.id: card for card in (*INITIAL_CARDS, *AGE_A_CARDS)}
    return CardDB(
        cards=cards,
