...
> **执行期修正记录（P0 终审确认）:** 本计划 Task 7 的 `_refill_row` 参考实现存在缺陷——补牌格数多于下一时代牌堆时，while 循环会把 `age` 推进到空牌堆的时代。实际实现改为游标探测、仅在启用非空牌堆时才提交 age 推进，并由 `test_age_skips_empty_middle_deck` 锁定。后续阶段不要按本节旧伪代码回退该修正。另外：P1 引入事件牌/军事牌堆洗牌前，须在 P1 计划中先明确 `rng_state` 的消费规范（何时推进、谁消费），防止确定性链条断裂。
